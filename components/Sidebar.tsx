...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const getMappedContent = (systemKey: string) => {
      // 1. Find ANY ID in mapping (prioritize bound layers)
      const mappedId = Object.keys(layerMapping).find(id => layerMapping[id] === systemKey);
      if (mappedId) {
  ...
  const updateMappedContent = (systemKey: string, val: string) => {
      ...
      if (!layerMapping[systemKey] || layerMapping[systemKey] === systemKey) {
          const fallbackLayer = layers.find(l => l.id === systemKey);
  ...
  // --- SUMMARIZE LOGIC (DETERMINISTIC: 3 Words + ...) ---
  const handleSummarize = (e: React.MouseEvent) => {
      ...
      // Fallback: If map returned nothing, look specifically for the tasting-notes layer
      if (!currentText) {
      ...
      // 2. Deterministic: Split by space, take first 3 words, add ...
