...
const InteractiveLayer: React.FC<{
...
}> = ({ layer, isSelected, onSelect, onUpdatePosition, onUpdateSize, viewMode, children }) => {
  ...
  const canDrag = isArchitecture || (isBatch && (layer.id.startsWith('badge') || layer.type === 'group' || layer.type === 'image'));
  
  // RESIZE PERMISSIONS
  // Architecture: All layers resizable
  // Batch: Only images resizable (for custom badges)
  const canResize = isArchitecture || (isBatch && layer.type === 'image');
  ...
  return (
    ...
    >
        <div className={`w-full h-full relative ${isSelected && (isArchitecture || layer.type === 'image') ? 'outline outline-1 outline-blue-500 shadow-xl border-dashed border border-blue-400' : (canDrag ? 'hover:outline hover:outline-1 hover:outline-blue-500/30' : '')}`}>
